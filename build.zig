...
pub fn build(b: *std.Build) void {
    ...
    // Configure Wayland scanner
    const scanner = Scanner.create(b, .{});
    const wayland_module = b.createModule(.{
        .root_source_file = scanner.result,
    });

    // Add River custom protocols
    ...
    // Generate standard Wayland client interfaces
    scanner.generate("wl_compositor", 4);
    scanner.generate("wl_shm", 1);
    scanner.generate("wl_output", 4);
    scanner.generate("wl_seat", 7);

    // Generate River window manager interfaces
    scanner.generate("river_window_manager_v1", 4);

    // Generate other River interfaces
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            // root module.
            .imports = &.{
                // Here "zinex" is the name you will use in your source code to
                // import this module (e.g. `@import("zinex")`). The name is
                // repeated because you are allowed to rename your imports, which
                // can be extremely useful in case of collisions (which can happen
                // importing modules from different packages).
                .{ .name = "zinex", .module = mod },
                .{ .name = "wayland", .module = wayland_module },
            },
        }),
    });
